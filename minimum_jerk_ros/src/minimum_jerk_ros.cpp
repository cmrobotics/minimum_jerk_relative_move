...
namespace minimum_jerk
{
    ...
    rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn MinimumJerkRos::on_configure(const rclcpp_lifecycle::State &)
    {
        ...
        collision_client_node_ = rclcpp::Node::make_shared("collision_client_node");
        collision_srv_client_ = collision_client_node_->create_client<cmr_msgs::srv::IsCollisionFree>("is_collision_free");
    ...
    rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn MinimumJerkRos::on_cleanup(const rclcpp_lifecycle::State &)
    {
        ...
        this->collision_client_node_.reset();
    ...
    void MinimumJerkRos::rotation_callback()
    {
        ...
        for (auto vel : odometry.get_velocities())
        {
            ...
            if (goal->enable_data_save)
            {
                ...
                FilFile::fil_one_ligne(fd_r_s_times, i, double((get_clock()->now() - t_init_for).nanoseconds()) * 1e-9, odometry.get_timestamps().at(i));
            }
            i++;
        }
    ...
    bool MinimumJerkRos::is_collision_free_(const geometry_msgs::msg::PoseStamped &pose)
    {
        ...
        if (!collision_srv_client_->wait_for_service(0.5s))
        {
            if (!rclcpp::ok())
            {
                RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Interrupted while waiting for the collision server response. Exiting.");
                return false;
            }
            RCLCPP_ERROR(rclcpp::get_logger("collision_client_node"), "collision server not available, the robot will stop...");
            return false;
        }

        auto result = collision_srv_client_->async_send_request(request);
        // Wait for the result.
        if (rclcpp::spin_until_future_complete(collision_client_node_, result, std::chrono::seconds(5)) !=
            rclcpp::FutureReturnCode::SUCCESS)
        {
            RCLCPP_ERROR(rclcpp::get_logger("collision_client_node"), "Failed to contact collision server");
            return false;
        }

        if (!result.get()->success)
        {
            RCLCPP_ERROR(rclcpp::get_logger("collision_client_node"), "Failed to get costmaps' status");
            return false;
        }
    ...
    std::vector<geometry_msgs::msg::PoseStamped> MinimumJerkRos::convert_poses_to_posestampeds_(std::vector<minimum_jerk::Pose> poses)
    {
        ...
        for (minimum_jerk::Pose &pose : poses)
        {
            ...
            current.pose.orientation = to_quaternion_(pose.get_theta(), 0, 0);
    ...
    geometry_msgs::msg::Quaternion MinimumJerkRos::to_quaternion_(double yaw, double pitch, double roll) // yaw (Z), pitch (Y), roll (X)
    {
    ...
    Trajectory MinimumJerkRos::compute_translation_velocities_(const std::shared_ptr<const minimum_jerk_msgs::action::Translate::Goal> &goal)
    {
        ...
        if (goal->enable_data_save)
        {
            FilFile::fil_file_translation<Pose>(odometry.get_timestamps(), odometry.get_poses(), "./src/mypackage/minimum_jerk_relative_move/minimum_jerk_trajectory_planner/ressources/poses.txt");
            FilFile::fil_file_translation<Velocity>(odometry.get_timestamps(), odometry.get_velocities(), "./src/mypackage/minimum_jerk_relative_move/minimum_jerk_trajectory_planner/ressources/velocities.txt");
            FilFile::fil_file_translation<Acceleration>(odometry.get_timestamps(), odometry.get_accelerations(), "./src/mypackage/minimum_jerk_relative_move/minimum_jerk_trajectory_planner/ressources/accelerations.txt");
        }
        return odometry;
    }
    ...
    void MinimumJerkRos::stop_translation_(geometry_msgs::msg::Twist linear_vel)
    {
    ...
    void MinimumJerkRos::stop_rotation_(geometry_msgs::msg::Twist angular_vel)
    {
